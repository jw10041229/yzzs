 ...
 * 
 */
public class HkPhChart extends AbstractDemoChart {
	public static String[] x;// x是用来显示的。xx是用来计算的
	private List<Da_ph> phList;
	double[] xx;
	private double[] phY;
	...
	private int cgqSx;
	public HkPhChart(int cgqSx) {//传感器顺序
		this.cgqSx = cgqSx;
	}
	public HkPhChart() {
	}
	...
	public LineChart execute(Context context) {
		phList = IndexActivitySupport.loadAllPhListBySx(context,String.valueOf(cgqSx));
		...
		x = new String[phList.size()];
		xx = new double[phList.size()];
		phY = new double[phList.size()];
		for (int i = 0; i < phList.size(); i++) {
			x[i] = (phList.get(i).getHksj());
			xx[i] = i;
			phY[i] = Double.valueOf(df00.format(Double.valueOf(phList
					.get(i).getHkph())));
		}
		if (phY.length > 0) {
			max =YzzsCommonUtil.getMaxMin(phY, 1);
			min =YzzsCommonUtil.getMaxMin(phY, 0);
		}
		
		String[] titles = new String[] { "PH—"+ labelTitle};
		...
		setChartSettings(renderer, "PH", "时间", "Ph", 0, phList.size(), min -1,
