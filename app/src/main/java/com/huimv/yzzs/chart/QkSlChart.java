 ...
 * 
 */
public class QkSlChart extends AbstractDemoChart {
	public static String[] x;// x是用来显示的。xx是用来计算的
	private List<Da_qk_shuiliu> shuiliuList;
	double[] xx;
	...
	private int cgqSx;
	public QkSlChart(int cgqSx) {//传感器顺序
		this.cgqSx = cgqSx;
	}
	public QkSlChart() {
	}
	...
	public LineChart execute(Context context) {
		shuiliuList = IndexActivitySupport.qk_LoadAllShuiliuListBySx(context,String.valueOf(cgqSx));
		...
		x = new String[shuiliuList.size()];
		xx = new double[shuiliuList.size()];
		wenduY = new double[shuiliuList.size()];
		for (int i = 0; i < shuiliuList.size(); i++) {
			x[i] = (shuiliuList.get(i).getQksj());
			xx[i] = i;
			wenduY[i] = Double.valueOf(df00.format(Double.valueOf(shuiliuList
					.get(i).getQksl())));
		}
		if (wenduY.length > 0) {
		...
		String[] titles = new String[] { "水流—"+ labelTitle};
		...
		setChartSettings(renderer, "水流", "时间", "水流", 0, shuiliuList.size(), min -1,
