 ...
 * 加热器开启温度设置
 */
public class HkJrqTempFragment extends YzzsBaseFragment implements EventHandler ,ConnectTimeoutListener,OnClickListener{
	...
	public View onYzzsInitView(LayoutInflater inflater, ViewGroup rootView, Bundle savedInstanceState) {
		View view = inflater.inflate(R.layout.activity_hk_jrq_temp_fragment, null);
	...
	private void readTemp() {
		...
		showLoading("正在读取...");
		sendGetBaseCMD(HkCMDConstant.HK_SBBZ,HkCMDConstant.READ_JRQ_TEMP);
	}
	...
	private void sendSaveTemp() {
		...
		String etTemp_2 = YzzsCommonUtil.formatStringAdd0
			(et_temp_2.getText().toString().trim().equals("") ? "0" : et_temp_2.getText().toString(), 2, 1);
		...
		paramSb.append(etTemp_1).append(startTemp1).append(closeTemp1)
				.append(etTemp_2).append(startTemp2).append(closeTemp2);

		int etDecimalLenth = paramSb.length();
		int lenth = etDecimalLenth + 12;
		int cmd = HkCMDConstant.SAVE_JRQ_TEMP;
		...
		Logger.d("byteData", Arrays.toString(byteData));
	}
	@Override
	public void onPause() {
	...
	private void getJoiningPack (byte[] message) {
		if(message[0] == HkCMDConstant.HK_SBBZ) {//环控
			try {
				...
				switch (cmd) {
					case HkCMDConstant.READ_JRQ_TEMP:
						getReadResult(message);
						break;
					case HkCMDConstant.SAVE_JRQ_TEMP:
		...
		if(message[5] == 0) {
			...
			if(message[6] == 0) {
				if (message [7] == 12) {
					...
					et_start_temp_1.setText(String.valueOf(Integer.valueOf(resultParam.append(message [10]).append(message [11]).toString())));
					resultParam.setLength(0);
					et_close_temp_1.setText(String.valueOf(Integer.valueOf(resultParam.append(message [12]).append(message [13]).toString())));

					resultParam.setLength(0);
					et_temp_2.setText(String.valueOf(Integer.valueOf(resultParam.append(message [14]).append(message [15]).toString())));
					resultParam.setLength(0);
					et_start_temp_2.setText(String.valueOf(Integer.valueOf(resultParam.append(message [16]).append(message [17]).toString())));
					resultParam.setLength(0);
					et_close_temp_2.setText(String.valueOf(Integer.valueOf(resultParam.append(message [18]).append(message [19]).toString())));
				} else {
