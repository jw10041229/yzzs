 ...
 *
 */
public class HkJqidFragment extends YzzsBaseFragment implements EventHandler ,ConnectTimeoutListener{
		...
		if (backData[0] == HkCMDConstant.HK_SBBZ) {// 环控
			try {
				...
				if (cmd == GeneralCMDConstant.READ_JQID) {
					getDqjqidResult(backData);
				}

				// 修改机器ID
				if (cmd == GeneralCMDConstant.CHANGE_JQID) {
					getXgjqidResult(backData);
				}
	...
	private void getXgjqidResult(byte[] backData) {
		if (backData[6] == 0) {
			isTimeOut = false;
			dismissLoading();
			ToastMsg(getActivity(), "修改机器ID成功!,请根据需要修改舍别名");
		} else { // 重启设备失败
