...
import static com.huimv.yzzs.activity.IndexActivity.FLZ_SBTS_FRAGMENT;

/**
 * 分栏站密码验证
 * @author jiangwei
 */
public class FlzMmyzFragment extends YzzsBaseFragment implements OnClickListener,EventHandler,ConnectTimeoutListener{
	//private final static String TAG = FlzMmyzFragment.class.getSimpleName();
	...
	public View onYzzsInitView(LayoutInflater inflater, ViewGroup rootView,
		...
		View view = inflater.inflate(R.layout.activity_flz_mmyz_fragment, null);
	...
	private void initData() {
		...
		String password = mSpUtil.getFlzPassword();
	...
	private void toSbtsFragment() {
		...
		FlzSbtsFragment mFlz_sbtsFragment= new FlzSbtsFragment();
		Bundle bundle = new Bundle();
		bundle.putString("identity", identity);
		mFlz_sbtsFragment.setArguments(bundle);
		ft.replace(R.id.fragment_container, mFlz_sbtsFragment,FLZ_SBTS_FRAGMENT);
	...
	private void receivePack(byte[] message) {
		...
		if(message[0] == FlzCMDConstant.FLZ_SBBZ) {//分栏站
			try {
				...
				switch (cmd) {
				case FlzCMDConstant.FLZ_MMYZ://密码验证
	...
	private void mmyzDataParsing(byte[] message) {
		...
		if (message.length >= 7) {
			...
			if (message [6] == 0 && message.length >= 16) {//密码验证成功
				...
				mSpUtil.setFlzPassword(et_password_input.getText().toString().trim());
				dismissLoading();
				toSbtsFragment();
			} else {
				dismissLoading();
	...
	private void sendLoginCMD() {
		...
		int cmd = FlzCMDConstant.FLZ_MMYZ;
				...
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if(YzzsApplication.isConnected) {
							...
							sendGetBaseCMD(FlzCMDConstant.FLZ_SBBZ, GeneralCMDConstant.RESTORE_FACTORY_SET);
						} else {
