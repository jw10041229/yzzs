 ...
 *
 */
public class LcJqidFragment extends YzzsBaseFragment implements EventHandler ,ConnectTimeoutListener{
	...
	private void sendReadJqid(int CmdType){
		...
		byteSendData[2] = YzzsCommonUtil.intTobyte(LcCMDConstant.LC_SBBZ); // 数据域：类型
	...
	public void onMessage(byte[] message) {
		if (message[0] == LcCMDConstant.LC_SBBZ) {// 分栏站
			try {
				...
				dismissLoading();
				ToastMsg(getActivity(), "lc_jqidFragment 数据异常");
			}
		}
	}
