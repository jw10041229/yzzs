...
import com.huimv.yzzs.util.wheel.FlzDevDebugItemSelectWheelUtil;
 ...
 *
 */
public class FlzSbtsFragment extends YzzsBaseFragment implements OnClickListener,
EventHandler,FlzDevDebugItemSelectWheelUtil.onDevDebugConfirmClickListener,FlzSetItemSelectIsShowWheelUtil.OnItemConfirmClickSetListener,ConnectTimeoutListener{
	private final StringBuffer dataStr = new StringBuffer();
	private FlzDevDebugItemSelectWheelUtil mFlz_devDebug_ItemSelectWheelUtil;
	...
	public View onYzzsInitView(LayoutInflater inflater, ViewGroup rootView,
		...
		View view = inflater.inflate(R.layout.activity_flz_sbts_fragment, null);
	...
	private void initData() {
		...
		mFlz_setItemSelectIsShowWheelUtil = new FlzSetItemSelectIsShowWheelUtil(getActivity(), "请选择子项目",Integer.valueOf(identity),FlzSbtsFragment.this);
		mFlz_devDebug_ItemSelectWheelUtil = new FlzDevDebugItemSelectWheelUtil(getActivity(), "请选择调试项目",FlzSbtsFragment.this);
		tvStateIsDebug();
	}
	...
	public void onResume() {
		...
		if (mSpUtil.getZjtdfx().equals(FlzCMDConstant.RIGHT_CHANNEL_SP)) {//右直角
			iv_sensor_location.setBackground(getResources().getDrawable(R.drawable.flz_right_channel_icon));
		}
		if (mSpUtil.getZjtdfx().equals(FlzCMDConstant.LEFT_CHANNEL_SP)) {//左直角
			iv_sensor_location.setBackground(getResources().getDrawable(R.drawable.flz_left_channel_icon));
		}
		super.onResume();
	}
	...
	public void onDevDebugConfirm(int position) {
		String pistionName = getResources().getStringArray(R.array.flz_debug_wheel_string_item_array)[position];
		int cmd = -1;
		if (pistionName .equals("打开入口门")) {
			cmd = FlzCMDConstant.FLZ_DKRKM;
		}
		if (pistionName .equals("关闭入口门")) {
			cmd = FlzCMDConstant.FLZ_GBRKM;
		}
		if (pistionName .equals("打开左通道")) {
			cmd = FlzCMDConstant.FLZ_DKZTD;
		}
		if (pistionName .equals("打开右通道")) {
			cmd = FlzCMDConstant.FLZ_DKYTD;
		}
		if (pistionName .equals("关闭旋转门")) {
			cmd = FlzCMDConstant.FLZ_GBXZM;
		}
		if (pistionName .equals("喷气驱赶")) {
			cmd = FlzCMDConstant.FLZ_PQQG;
		}
		if (pistionName .equals("打开分离门")) {
			cmd = FlzCMDConstant.FLZ_DKFLM;
		}
		if (pistionName .equals("关闭分离门")) {
			cmd = FlzCMDConstant.FLZ_GBFLM;
		}
		if (pistionName .equals("喷墨一")) {
			cmd = FlzCMDConstant.FLZ_PM1;
		}
		if (pistionName .equals("喷墨二")) {
			cmd = FlzCMDConstant.FLZ_PM2;
		}
		if (pistionName .equals("快速调试")) {
			cmd = FlzCMDConstant.FLZ_KSTS;
		}
		if (pistionName .equals("称重调试")) {
			toFragment(new FlzCztsFragment());
		}
		if (pistionName .equals("RFID调试")) {
			toFragment(new FlzRFIDtsFragment());
		}
		if (pistionName .equals("硬件设置")) {
			toFragment(new FlzYjszFragment());
		}
		if (pistionName .equals("RFID出栏")) {
			toFragment(new FlzRFIDClFragment());
		}
		if (cmd != -1) {
			if (cmd == FlzCMDConstant.FLZ_KSTS) {
				showLoading("正在快速调试...");
	...
	public void onClick(View v) {
		...
		case R.id.tv_manual_debug:
			if(YzzsApplication.isConnected) {
				if (IndexActivity.flz_work_isDebuging) {//如果是调试模式,怎直接运行调试项目
					mFlz_devDebug_ItemSelectWheelUtil.showDialog();
	...
	private void prepareDebug() {
		showLoading("正在准备调试...");
		sendGetBaseCMD(FlzCMDConstant.FLZ_SBBZ, FlzCMDConstant.FLZ_ZBTS);
	}
	/**
	 * 是否进入调试模式
	 */
	...
	private void doExitDebug() {
		showLoading("正在退出调试...");
		sendGetBaseCMD(FlzCMDConstant.FLZ_SBBZ, FlzCMDConstant.FLZ_TCTS);
	}
	...
	private void receivePack(byte[] message) {
		...
		if(message[0] == FlzCMDConstant.FLZ_SBBZ) {//分栏站
			try {
				...
				switch (cmd) {
				case FlzCMDConstant.FLZ_KSTS://快速调试
					kstsDataParsing(message);
					break;
				case FlzCMDConstant.FLZ_DKRKM://打开入口门
				case FlzCMDConstant.FLZ_GBRKM://关闭入口门
				case FlzCMDConstant.FLZ_DKZTD://打开左通道
				case FlzCMDConstant.FLZ_DKYTD://打开右通道
				case FlzCMDConstant.FLZ_GBXZM://关闭旋转门
				case FlzCMDConstant.FLZ_PQQG://喷气驱赶
				case FlzCMDConstant.FLZ_DKFLM://打开分离门
				case FlzCMDConstant.FLZ_GBFLM://关闭分离门
					sensorStatePausing(message);
					break;
				case FlzCMDConstant.FLZ_PM1://喷墨1
				case FlzCMDConstant.FLZ_PM2://喷墨2
					pmStatePausing(message);
					break;
				case FlzCMDConstant.FLZ_TCTS://退出调试
					exitDebugPausing(message);
					break;
				case GeneralCMDConstant.RESTART_XT_DEVICE://设备重启
					restartDev(message);
					break;
				case FlzCMDConstant.FLZ_ZBTS://进入调试模式
	...
	private void doPreparePausing(byte[] message) {
		if (message[5] == 0) {
			...
			isTimeOut = false;
			if (message[6] == 0) {// 动作
				IndexActivity.flz_work_isDebuging = true;
				tvStateIsDebug();
				mFlz_devDebug_ItemSelectWheelUtil.showDialog();
			} else { // 动作
	...
	private void exitDebugPausing(byte[] message) {
		if (message[5] == 0) {
			...
			isTimeOut = false;
			if (message[6] == 0) {// 动作
				IndexActivity.flz_work_isDebuging = false;
				tvStateIsDebug();
	...
	private void sendRestartDeviceCMD() {
		sendGetBaseCMD(FlzCMDConstant.FLZ_SBBZ, GeneralCMDConstant.RESTART_XT_DEVICE);
	}
	...
	private void toSetFragment(Fragment mFragment) {
		...
		bundle.putInt("whichDev", FlzCMDConstant.FLZ_SBBZ);
