 ...
 * @author jiangwei
 */
public class LcMmyzFragment extends YzzsBaseFragment implements OnClickListener,EventHandler,ConnectTimeoutListener{
	//private final static String TAG = LcMmyzFragment.class.getSimpleName();
	...
	public View onYzzsInitView(LayoutInflater inflater, ViewGroup rootView,
		...
		View view = inflater.inflate(R.layout.activity_lc_mmyz_fragment, null);
	...
	private void toSbtsFragment() {
		...
		LcCsszFragment mLc_cssz_Fragment= new LcCsszFragment();
		Bundle bundle = new Bundle();
		bundle.putString("identity", identity);
		mLc_cssz_Fragment.setArguments(bundle);
		ft.replace(R.id.fragment_container, mLc_cssz_Fragment,"lc_cssz_Fragment");
	...
	private void receivePack(byte[] message) {
		...
		if(message[0] == LcCMDConstant.LC_SBBZ) {//料槽
			try {
				...
				switch (cmd) {
				case LcCMDConstant.LC_MMYZ://密码验证
	...
	private void sendLoginCMD() {
		...
		byteData[2] = YzzsCommonUtil.intTobyte(LcCMDConstant.LC_SBBZ);//设备区分标志
				...
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if(YzzsApplication.isConnected) {
							...
							sendGetBaseCMD(LcCMDConstant.LC_SBBZ, GeneralCMDConstant.RESTORE_FACTORY_SET);
						} else {
