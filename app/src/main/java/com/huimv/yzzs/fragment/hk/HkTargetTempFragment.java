 ...
 * 加热器开启温度设置
 */
public class HkTargetTempFragment extends YzzsBaseFragment implements EventHandler ,ConnectTimeoutListener,OnClickListener{
	...
	public View onYzzsInitView(LayoutInflater inflater, ViewGroup rootView, Bundle savedInstanceState) {
		View view = inflater.inflate(R.layout.activity_hk_target_temp_fragment, null);
	...
	private void readTemp() {
		...
		showLoading("正在读取...");
		sendGetBaseCMD(HkCMDConstant.HK_SBBZ,HkCMDConstant.READ_TARGET_TEMP);
	}
	...
	private void sendSaveTemp() {
		...
		int cmd = HkCMDConstant.SAVE_TARGET_TEMP;
	...
	private void getJoiningPack (byte[] message) {
		if(message[0] == HkCMDConstant.HK_SBBZ) {//环控
			try {
				...
				switch (cmd) {
					case HkCMDConstant.READ_TARGET_TEMP:
						getReadResult(message);
						break;
					case HkCMDConstant.SAVE_TARGET_TEMP:
