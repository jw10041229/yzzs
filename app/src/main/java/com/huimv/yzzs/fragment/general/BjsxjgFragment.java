 ...
 *
 */
public class BjsxjgFragment extends YzzsBaseFragment implements OnClickListener,
	 ...
	 * 输入框
	 */
	private EditText et_time_confim_value;
	...
	public View onYzzsInitView(LayoutInflater inflater, ViewGroup rootView, Bundle savedInstanceState) {
		View view = inflater.inflate(R.layout.activity_bjsxjg_fragment, rootView,false);
		initView(view);
		initData();
		return view;
	}
	...
	private void initData() {
		...
		whichDevSaveCMD = GeneralCMDConstant.SAVE_DD_QRSJ;
		whichDevReadCMD = GeneralCMDConstant.READ_DD_QRSJ;
	...
	private void initView(View view) {
		...
		et_time_confim_value = (EditText) view.findViewById(R.id.et_time_confim_value);
	}
	...
	public void onClick(View v) {
		...
		case R.id.btn_save:
			if(YzzsApplication.isConnected) {
				...
				sendXgQrsjCmd();
				startTime(HkCMDConstant.SEND_HK_SAVE_CMD_TIMEOUT, "时间保存");
				isTimeOut = true;
			} else {
		...
		case R.id.btn_read:
			if(YzzsApplication.isConnected) {
				...
				startTime(HkCMDConstant.SEND_HK_SAVE_CMD_TIMEOUT, "读取时间");
				isTimeOut = true;
			} else {
				ToastMsg(getActivity(), getString(R.string.disconnected));
			}
	...
	private boolean checkData() {
		boolean isOK;
		String inputData = et_time_confim_value.getText().toString().trim();
		if (CommonUtil.isEmpty(inputData) || Integer.valueOf(inputData) < 20) {
			ToastMsg(getActivity(), "确认时间请输入20--999秒之内的值");
			isOK = false;
		} else {
			isOK = true;
		}
	...
	private void getReadResult(byte[] message) {
		if (message [5] == 0) {
			...
			if(message[6] == 0) {
				...
				et_time_confim_value.setText(Integer.valueOf(sb.toString()) + "");
			} else {
				ToastMsg(getActivity(), "时间读取失败");
			}
		}
	}
	/**
	 ...
	 * 修改确认时间蓝牙名称
	 */
	private void sendXgQrsjCmd() {
		String qrsjStr = et_time_confim_value.getText().toString().trim(); // 输入确认时间
		qrsjStr = YzzsCommonUtil.formatStringAdd0(qrsjStr, 3, 1);
		int singleDataLength = qrsjStr.length(); // 单条有效数据的长度
		// byte 数据的总长度
		...
		for (int i = GDW_LEN_HEAD; i < len; i++) {
			int data = Integer.valueOf((qrsjStr.substring(i - GDW_LEN_HEAD, i - GDW_LEN_HEAD + 1)));
