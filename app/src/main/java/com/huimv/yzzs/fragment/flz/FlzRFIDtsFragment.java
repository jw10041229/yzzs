 ...
 *
 */
public class FlzRFIDtsFragment extends YzzsBaseFragment implements EventHandler,ConnectTimeoutListener{
	//private final static String TAG = FlzRFIDtsFragment.class.getSimpleName();
	/**
	 * read_rfid按钮
	 */
	...
	public View onYzzsInitView(LayoutInflater inflater, ViewGroup rootView,
		...
		View view = inflater.inflate(R.layout.activity_flz_rfidts_fragment, null);
		initView(view);
		return view;
	}
		...
		btn_rfid_read.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if(YzzsApplication.isConnected) {
					...
					sendGetBaseCMD(FlzCMDConstant.FLZ_SBBZ, FlzCMDConstant.FLZ_RFIDTS);
	...
	private void receivePack(byte[] message) {
		...
		if(message[0] == FlzCMDConstant.FLZ_SBBZ) {//分栏站
			try {
				...
				switch (cmd) {
				case FlzCMDConstant.FLZ_RFIDTS://耳标
