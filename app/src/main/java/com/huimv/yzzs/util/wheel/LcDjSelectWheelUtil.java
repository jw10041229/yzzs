...
public class LcDjSelectWheelUtil {
	...
	public LcDjSelectWheelUtil(Activity context, String titleStr, String[] item,
							   OnDjConfirmClickListener mOnConfirmClickListener) {
		this.context = context;
		this.titleStr = titleStr;
		this.item = item;
		this.mOnDjConfirmClickListener = mOnConfirmClickListener;
	}

	// 创建接口
	public interface OnDjConfirmClickListener {
		void onDjConfirm(int position, int i);
	}

	// 声明接口对象
	private OnDjConfirmClickListener mOnDjConfirmClickListener;
		...
		new AlertDialog.Builder(context).setView(view).setTitle(titleStr)
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						if (mOnDjConfirmClickListener != null) {
							mOnDjConfirmClickListener.onDjConfirm(expenses_id, i);
						}
						dialog.dismiss();
					}
